...
const char deviceName[] PROGMEM = "INTERRUPT";  // The Device friendly name
const char deviceType[] PROGMEM = "INTRPT001";  // The user defined device type
        ...
        else
        {
          ...
          if (timeout > 0) {
            ...
            if (currentMillis - previousMillis >= timeout) {
              ...
              readAnalog(limit, total, trigger);
            }
          }
	        else {
            // calculate sumations
            if (readAnalog(limit, total, trigger)){
              LLAP.sendInt("COUNT", total); // echo back the instruction
              total = 0;
            }
          }
 ...
 * Read Analogue pin
 */
 bool readAnalog(int threshold, int &count, bool &trigger) {
  long result = false;
  threshold = 25;
  int pin = A0;
...
void saveLimit(int threshold)
{
  ...
  if (temp != threshold)
  {
    ...
    EEPROM.put(EEPROM_LIMIT_ADDRESS, threshold);
  }
}
...
int loadLimit() {
  ...
  LLAP.sendInt("INTVL+", limit); // send back error status
  return (limit);
}
